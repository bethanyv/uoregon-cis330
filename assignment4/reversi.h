...
class BoardType {
	public:
		...
		BoardType(int size); // Matrix of pieces here

		// run the game
		void runGame(); // ask players for what they want to do, place pieces
		...
		int placePiece(const piece color, const int x, const int y);
		...
		int verticalValid(const int start_index_i, const int start_index_j, const piece color);

		// check if there is a valid horizontal move - if it is, return the 
		// index of other piece closing off opponent's pieces. If not, return -1
		int horizontalValid(const int start_index_i, const int start_index_j, const piece color);
		...
		indexType leftUpDiagonalValid(const int x, const int y, const piece color);

		// right up checks the diagonal right up and left down
		indexType rightUpDiagonalValid(const int x, const int y, const piece color);
		...
		int anyValidMoves(const piece color);

		// flip the pieces in between the indexes
		void horizontalFlip(const int start_index_j, const int end_index_j, const int index_i, const piece color);

		// flip the pieces in between the indexes
		void verticalFlip(const int start_index_i, const int end_index_i, const int index_j, const piece color);

		// flip the pieces in between the indexes
		void diagonalFlip(const indexType from, const indexType to, const piece color);
