...
void runGame(BoardType *board) {
	int x, y;
	while(1) {// TO DO WHILE COLOR HAS LEGAL MOVES
		printBoard(board);
		printf("%s", "\nPlease input space you want to go to: ");
		scanf("%d %d", &x, &y);
		...
		while(!boo) { // TODO: UPDATE TO !BOO AND AVAILABLE SPACES
...
int placePiece(const BoardType *board, const piece color, const int x, const int y) {
	...
	// 	diagonalFlip(board, diagonal_valid[0], diagonal_valid[1], x, y, color);
	// 	free(diagonal_valid);
	// 	boo = 1;
	// }
...
int horizontalValid(const BoardType *board, const int start_index_i, const int start_index_j, const piece color) {
	...
	if(increasing < board->size && increasing > -1) {
		...
		while(board->pieces[start_index_i][increasing] != color) {
			...
			if(board->pieces[start_index_i][increasing] == empty || increasing == board->size) {
